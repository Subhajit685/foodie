...
function Login() {
  ...
  const {islogin, login} = useContext(storeContext)
        ...
        <div className='w-full max-w-md p-8 space-y-6 bg-white/60 rounded-lg shadow-md'>
          ...
          <form onSubmit={handelSubmit}>
            ...
            <button className='text-center broder h-8 w-full bg-red-600 my-1 rounded hover:bg-red-700 hover:text-white flex justify-center items-center'>
              Submit{islogin ? <Spinner/> : ""}
            </button>
            <div className='py-1 text-lg'>
              <span>New user click here to register first ? </span><span><Link to={"/singup"} className='text-blue-700 hover:underline'>Sing up</Link></span>
            </div>
          </form>
